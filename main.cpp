...
const string day_names[NUMBER_OF_DAYS] = {"Saturday", "Sunday",    "Monday",
                                          "Tuesday",  "Wednesday", "Thursday",
                                          "Friday"};
const string period_times[NUMBER_OF_PERIODS][2] = {
    {"7:30", "9:00"}, {"9:30", "11:00"}, {"11:30", "13:00"}};
...
void match_available_periods(course *current_course, day *current_day, int st,
                             int fn);
...
time_segment *find_matching_period(course *current_course,
                                   time_segment *current_period);
dars make_dars(teacher *instructor, course *subject, time_segment *period1,
    ...
    for (j = 0; j < NUMBER_OF_DAYS; j++) {
      for (k = 0; k < NUMBER_OF_PERIODS; k++) {
        ...
        for (course *x : days[j].periods[k].available_courses) {
          ...
          if (kelases[i].matched[period_1_it] ||
              kelases[i].matched[period_2_it]) {
            continue;
          }
          for (teacher *y : x->available_teachers) {
            vector<time_segment *>::iterator it1 =
                find(all(y->available_periods), period_1_it);
            vector<time_segment *>::iterator it2 =
                find(all(y->available_periods), period_2_it);
            if (it1 == (y->available_periods).end() ||
                it2 == (y->available_periods).end()) {
              continue;
            }
            vector<teacher *>::iterator teach_it1 =
                find(all((*it1)->available_teachers), y);
            vector<teacher *>::iterator teach_it2 =
                find(all((*it2)->available_teachers), y);
            if (teach_it1 == (*it1)->available_teachers.end() ||
                teach_it2 == (*it2)->available_teachers.end()) {
              continue;
            }
...
bool teacher_cmp(teacher *a, teacher *b) {
  int sza = (int)a->available_periods.size(),
      szb = (int)b->available_periods.size();
...
bool has_dars(kelas current_kelas, course *subject) {
  return find_if(all(current_kelas.doroos), [&](dars x) {
           return x.subject == subject;
         }) != current_kelas.doroos.end();
}
...
void print_dars_in_klass(course *course_ptr, kelas current_kelas) {
  vector<dars>::iterator it = find_if(all(current_kelas.doroos), [&](dars x) {
    return x.subject == course_ptr;
  });
  if (it == current_kelas.doroos.end()) {
    cout << "Not Found" << endl;
    return;
  }
  cout << it->instructor->name << ": " << int_to_time(it->period1->st) << " "
       << int_to_time(it->period1->fn) << endl;
}
